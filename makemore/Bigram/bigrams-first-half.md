...
- `char`就是得到的出现的所有字母，按照字母表顺序的到的列表
- `stoi(String TO Integer)`:构建字符到矩阵索引的映射`enumerate` 会吐出 `(0, 'a'), (1, 'b')...`这样`(索引，字符)的结构`。我们让**字母做`Key`，把索引` +1` 做 `Value`**（即 `'a': 1, 'b': 2`）,这里的`+1`是为了把0号位置留给特殊字符`.`
- `itos`键值反转，数字做Key，字母做Value，方便后续模型算出数字结果之后翻译回人类能看懂的字母
